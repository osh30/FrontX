            ...
            {announcements.map((a, i) => {
              ...
              return (
                <motion.div key={a._id} variants={fadeUp} custom={i} className="relative overflow-hidden rounded-2xl p-[1px] group">
                  <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-white/[0.08] via-white/[0.04] to-white/[0.07] group-hover:from-white/[0.14] group-hover:via-white/[0.08] group-hover:to-white/[0.12] transition-all duration-300" />
                  <div className="relative rounded-[calc(1rem-1px)] p-5 transition-all duration-300 hover:shadow-xl hover:shadow-black/20"
                    ...
                    <div className="absolute inset-0 pointer-events-none overflow-hidden rounded-[calc(1rem-1px)]">
                      <div className="absolute inset-0 opacity-[0.02] group-hover:opacity-[0.04] transition-opacity" style={{ background: 'linear-gradient(115deg, transparent 20%, rgba(148,163,184,0.5) 40%, rgba(255,255,255,0.7) 50%, rgba(148,163,184,0.5) 60%, transparent 80%)', backgroundSize: '250% 100%', animation: 'shimmerSweep 8s ease-in-out infinite' }} />
                    </div>
                    <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-5">
                      ...
                      <div className="flex-1 min-w-0 space-y-2">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {a.isPinned && (
                            <span className="flex items-center gap-1 text-[10px] font-bold px-2.5 py-0.5 rounded-md bg-amber-500/10 text-amber-400 border border-amber-500/20">
                        ...
                        <div>
                          <h3 className="text-[16px] font-bold text-white tracking-tight leading-snug group-hover:text-blue-300 transition-colors">
                            {a.title}
                          </h3>
                          <p className="text-[13px] text-slate-400 leading-relaxed mt-1 line-clamp-2">
                            {a.description}
                          </p>
                        </div>

                        <div className="flex items-center gap-4 text-[11px] text-slate-500 pt-0.5 flex-wrap">
                          <span className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5 text-slate-400" /> Published: {formatDate(a.publishDate)}</span>
                          {a.expiryDate && <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5 text-slate-400" /> Exp: {formatDate(a.expiryDate)}</span>}
                          <span className="text-slate-600">Posted by: {a.postedBy?.name || 'Administrator'}</span>
                          {a.attachment && (
                            <a href={a.attachment} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-blue-400 hover:underline">
                      ...
                      {/* Right side actions */}
                      <div className="flex items-center gap-2 shrink-0 md:border-l md:border-white/[0.08] md:pl-5 pt-3 md:pt-0 border-t border-white/[0.06] md:border-t-0 justify-end">
                        <button
                          onClick={() => setViewModal(a)}
                          className="flex items-center gap-1.5 px-3.5 py-2 text-xs font-semibold text-blue-400 bg-blue-500/10 hover:bg-blue-500/20 border border-blue-500/20 rounded-xl transition-all shadow-sm"
                          title="View Details"
                        >
                          <Eye className="w-3.5 h-3.5" /> View Details
                        </button>
                        <button
                          onClick={() => navigate(`/admin/announcements/edit/${a._id}`)}
                          className="p-2 text-slate-400 hover:text-amber-400 hover:bg-amber-500/[0.1] rounded-xl border border-white/[0.06] hover:border-amber-500/20 transition-all"
                        ...
                        <button
                          ...
                          className={`p-2 rounded-xl border border-white/[0.06] transition-all ${
                            a.isPinned
                              ? 'text-amber-400 bg-amber-500/10 border-amber-500/20'
                              : 'text-slate-400 hover:text-amber-400 hover:bg-amber-500/[0.1] hover:border-amber-500/20'
                          }`}
                        ...
                        <button
                          ...
                          className="p-2 text-slate-400 hover:text-red-400 hover:bg-red-500/[0.1] rounded-xl border border-white/[0.06] hover:border-red-500/20 transition-all"
