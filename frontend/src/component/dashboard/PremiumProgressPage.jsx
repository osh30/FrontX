...
const PremiumProgressPage = ({ setActiveTab }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-12 max-w-7xl mx-auto w-full">
      {/* ===== HERO SECTION ===== */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="grid lg:grid-cols-3 gap-6 mb-8">
        {/* Hero Left */}
        <div className="lg:col-span-2 bg-white rounded-3xl p-8 border border-gray-100 shadow-sm">
          <div className="flex flex-col md:flex-row items-start md:items-center gap-8">
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2">
                <div className="p-2 rounded-xl bg-purple-50"><TrendingUp className="w-5 h-5 text-purple-600" /></div>
                <span className="text-xs font-bold text-purple-600 bg-purple-50 px-3 py-1 rounded-full uppercase tracking-wider">Career Progress</span>
              </div>
              <h1 className="text-3xl font-black text-gray-900 mt-2">Your Career Journey</h1>
              <p className="text-gray-500 mt-2 text-sm max-w-lg">Track your growth, earn achievements, and unlock new career levels as you build your professional profile.</p>
              <div className="flex flex-wrap gap-6 mt-6">
                {[
                  { label: 'Total XP', value: xp, icon: Zap, color: 'text-yellow-500', bg: 'bg-yellow-50' },
                  { label: 'Current Streak', value: currentStreak, icon: Flame, color: 'text-orange-500', bg: 'bg-orange-50', suffix: ' days' },
                  { label: 'Rank', value: `#${rank}`, icon: Crown, color: 'text-amber-500', bg: 'bg-amber-50', suffix: ` / ${totalStudents}` },
                  { label: "Today's Activity", value: todaysProductivity, icon: Activity, color: 'text-blue-500', bg: 'bg-blue-50', suffix: ' actions' }
                ].map((item, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <div className={`p-2.5 rounded-xl ${item.bg}`}><item.icon className={`w-4 h-4 ${item.color}`} /></div>
                    <div>
                      <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">{item.label}</p>
                      <p className="text-lg font-black text-gray-900">
                        <AnimatedCounter value={typeof item.value === 'number' ? item.value : parseInt(item.value) || 0} />
                        {item.suffix || ''}
                      </p>
                    </div>
                  </div>
            ...
            <div className="shrink-0">
              <CircularProgress value={careerScore} max={1000} size={150} strokeWidth={10} sublabel="Score" color="url(#scoreGrad)" />
              <svg width="0" height="0"><defs><linearGradient id="scoreGrad" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="#8b5cf6" /><stop offset="100%" stopColor="#3b82f6" /></linearGradient></defs></svg>
              <p className="text-center text-xs font-bold text-gray-500 mt-1">{scorePercent}% Complete</p>
            </div>
          </div>
        </div>

        {/* Career Digital Twin */}
        <div className="relative overflow-hidden bg-gradient-to-br from-[#0F172A] via-[#1a2444] to-[#0F172A] rounded-3xl p-6 border border-white/5 shadow-xl">
          <div className="absolute -top-20 -right-20 w-40 h-40 bg-purple-500/10 rounded-full blur-3xl" />
          <div className="absolute -bottom-10 -left-10 w-32 h-32 bg-blue-500/10 rounded-full blur-3xl" />
          <div className="relative z-10 flex flex-col items-center text-center">
            <Avatar src={userProfile?.profilePicture} alt={userProfile?.name} size={80} className="border-4 border-purple-500/30 shadow-lg mb-3" />
            <h3 className="text-xl font-black text-white">{userProfile?.name || 'Student'}</h3>
            <p className="text-xs text-white/50 mb-4">{careerLevel} Level</p>
            <GlowingRing value={careerScore} max={1000} size={110} strokeWidth={6} color="#8b5cf6" />
            <div className="flex gap-4 mt-4 w-full justify-center">
              <div className="text-center">
                <p className="text-lg font-black text-white"><AnimatedCounter value={xp} /></p>
                <p className="text-[9px] font-bold text-white/40 uppercase tracking-wider">XP</p>
              </div>
              <div className="w-px bg-white/10" />
              <div className="text-center">
                <p className="text-lg font-black text-white">#{rank}</p>
                <p className="text-[9px] font-bold text-white/40 uppercase tracking-wider">Rank</p>
              </div>
            </div>
            <div className="w-full mt-4">
              ...
              <div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden">
                <motion.div className="h-full rounded-full bg-gradient-to-r from-purple-500 to-blue-500" initial={{ width: 0 }} animate={{ width: `${nextLevelProgress}%` }} transition={{ duration: 1.5, ease: 'easeOut' }} />
              </div>
            </div>
          </div>
        </div>
