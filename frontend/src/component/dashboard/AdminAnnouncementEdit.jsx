...
const AdminAnnouncementEdit = () => {
  ...
  const { id } = useParams();
  const [form, setForm] = useState({ title: '', category: 'Academic', priority: 'Normal', description: '', publishDate: '', expiryDate: '', attachment: '', isPinned: false });
  ...
  useEffect(() => {
    const fetchAnnouncement = async () => {
      try {
        const res = await axios.get(`${API_URL}/admin/announcements?page=1&limit=100`);
        const found = res.data.announcements.find(a => a._id === id);
        if (!found) { showToast('Announcement not found', 'error'); navigate('/admin/announcements'); return; }
        setForm({
          title: found.title,
          category: found.category,
          priority: found.priority,
          description: found.description,
          publishDate: found.publishDate ? new Date(found.publishDate).toISOString().slice(0, 16) : '',
          expiryDate: found.expiryDate ? new Date(found.expiryDate).toISOString().slice(0, 16) : '',
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await axios.put(`${API_URL}/admin/announcements/${id}`, payload);
      ...
      <motion.div variants={fadeUp} className="relative overflow-hidden rounded-2xl mx-8 mt-8 p-[1px]">
        ...
        <div className="relative rounded-[calc(1rem-1px)] overflow-hidden" style={{ background: 'linear-gradient(170deg, #0B1120 0%, #0F1B2D 25%, #111D33 50%, #0D1625 75%, #0A0F1E 100%)' }}>
          ...
          <div className="relative z-10 px-8 sm:px-10 py-10 sm:py-12">
            ...
            <h1 className="text-[32px] sm:text-[38px] font-[800] text-white tracking-[-0.03em] leading-[1.1]">Edit Announcement</h1>
            <p className="text-[17px] text-slate-400 leading-[1.6] max-w-lg mt-2">Update the announcement details below.</p>
          </div>
        </div>
      </motion.div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-[0_1px_3px_rgba(0,0,0,0.04),0_8px_24px_rgba(0,0,0,0.04)] overflow-hidden">
            <div className="p-8 sm:p-10 space-y-6">
              <div>
                ...
                <input type="text" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Announcement title" className={inputClass} />
              </div>
              ...
              <div>
                ...
                <textarea rows={6} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Write the full announcement details..." className={`${inputClass} resize-none`} />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
              ...
              <div>
                ...
                <input type="text" value={form.attachment} onChange={(e) => setForm({ ...form, attachment: e.target.value })} placeholder="https://... (PDF or image link)" className={inputClass} />
              </div>
            ...
            <div className="px-8 sm:px-10 py-5 flex items-center justify-between">
              ...
              <button onClick={handleUpdate} disabled={submitting || !form.title || !form.description}
                ...
                {submitting ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Saving...</> : 'Update Announcement'}
              </button>
            </div>
          </div>
        </motion.div>
