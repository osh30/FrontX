      ...
      <AnimatePresence>
        {viewModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" onClick={() => setViewModal(null)} />
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              className="relative w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl">
              <div className="relative rounded-2xl p-6 bg-white border border-slate-200">
                <div className="flex items-center justify-between mb-5">
                  <div className="flex items-center gap-2.5">
                    ...
                    {viewModal.featured && (
                      <span className="text-[11px] font-bold px-2 py-1 rounded-lg bg-amber-500/10 text-amber-600 border border-amber-500/20 flex items-center gap-1">
                        <Star className="w-3 h-3 text-amber-500" /> Featured
                      </span>
                    )}
                  </div>
                  <button onClick={() => setViewModal(null)} className="p-1.5 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-lg transition-all">
                    <X className="w-4 h-4" />
                  </button>
                </div>

                <h2 className="text-xl font-bold text-slate-900 mb-1">{viewModal.title}</h2>
                <div className="flex items-center gap-1.5 mb-4">
                  <Building2 className="w-3.5 h-3.5 text-slate-400" />
                  <span className="text-sm text-slate-600 font-medium">{viewModal.organization}</span>
                </div>

                <div className="space-y-4">
                  <div>
                    <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Description</p>
                    <p className="text-sm text-slate-700 leading-relaxed whitespace-pre-wrap">{viewModal.description}</p>
                  </div>
                  {viewModal.eligibility && (
                    <div>
                      <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Eligibility</p>
                      <p className="text-sm text-slate-700 leading-relaxed whitespace-pre-wrap">{viewModal.eligibility}</p>
                    </div>
                  )}
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <span className={`text-[11px] font-bold px-2.5 py-1 rounded-md ${
                        isExpired(viewModal.deadline) ? 'bg-red-50 text-red-600 border border-red-200' : 'bg-emerald-50 text-emerald-600 border border-emerald-200'
                      }`}>
              ...
              <div className="relative rounded-2xl p-6 bg-white border border-slate-200">
                ...
                <div className="flex items-center justify-between mb-6">
                  <div>
                    ...
                    <p className="text-sm text-slate-500 mt-0.5">Update the opportunity details below.</p>
                  </div>
                  <button onClick={closeModals} className="p-1.5 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-lg transition-all">
                    <X className="w-4 h-4" />
                  </button>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm text-slate-800 outline-none focus:ring-2 focus:ring-blue-500/15 focus:border-blue-400 shadow-sm transition-all"
                    >
                      {TYPES.slice(1).map(t => <option key={t} value={t} className="bg-white text-slate-800">{t}</option>)}
                    </select>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm text-slate-800 placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-blue-500/15 focus:border-blue-400 shadow-sm transition-all"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm text-slate-800 placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-blue-500/15 focus:border-blue-400 shadow-sm transition-all resize-none"
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between py-3 px-4 bg-slate-50 border border-slate-200 rounded-xl">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-slate-800">Featured Opportunity</p>
                    ...
                    <button
                      ...
                      className={`relative w-11 h-6 rounded-full transition-all duration-300 ${form.featured ? 'bg-blue-600' : 'bg-slate-200'}`}
                    >
                ...
                <div className="flex gap-3 mt-6">
                  <button onClick={closeModals} disabled={submitting}
                    className="flex-1 py-2.5 text-sm font-medium text-slate-600 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded-xl transition-all disabled:opacity-50">
                    Cancel
                  </button>
                  <button
